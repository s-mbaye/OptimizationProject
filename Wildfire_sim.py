...
list=[]
list.append(base(base_location))


class aircraft:
  def __init__(self, number, location, goal_loc, fuel_status, retardant_status, assigned_base, max_range, max_speed):
    self.number = number
    self.location = aircraft_location # starting location above
    self.goal_loc = (i,j) # starting goal at starting location for wildfire above
    self.fuel_status = 100 # starting fuel at 100%
    self.retardant_status = 100 # starting retardant at 100%
    self.assigned_base = 1
    self.max_range = 1287 # in km
    self.max_speed = 6.6  # in km/min

# create the list
aircraftL = []

# appending aircraft to the list
aircraftL.append(aircraft(1, aircraft_location, (i,j), 100, 100, 1, 1287, 6.6))
aircraftL.append(aircraft(2, aircraft_location, (i,j), 100, 100, 1, 1287, 6.6))
aircraftL.append(aircraft(3, aircraft_location, (i,j), 100, 100, 1, 1287, 6.6))

# Test by printing
for  obj in aircraftL:
  print(obj.number, obj.location, obj.goal_loc, obj.fuel_status, obj.retardant_status, obj.assigned_base, obj.max_range, obj.max_speed, sep='    ')

print("")
#accessing elements in the list
print(aircraftL[0].number)
print(aircraftL[1].number)
print(aircraftL[2].number)
...
for t in range(1,50):
   ...
   for ac in range(1,4):
    print('At this time Aircraft', ac, 'location is', aircraftL[ac-1].location)
    aircraftL[ac-1].location, aircraft_loaded, wildfire_map, wildfire_burnedQ  = simulate_aircraft(aircraftL[ac-1].location, base_location, aircraft_loaded, wildfire_map, wildfire_burnedQ)